...
def calculate_path_distance():
    ...
    if not len(sys.argv) >= 4:
        ...
        pub_goal = rospy.Publisher('/gopher_presence/move_base_simple/goal', PoseStamped, queue_size=10)
        pub_start = rospy.Publisher('/gopher_presence/initialpose', PoseWithCovarianceStamped, queue_size=10)
...
def printPath(path):
    ...
    if len(path.poses) > 0:
	if str(sys.argv[3]) == '0':
	        pub_stop = rospy.Publisher('/gopher_presence/move_base/cancel', GoalID, queue_size=10)
        	rospy.sleep(1)
